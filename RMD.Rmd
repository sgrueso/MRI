...
```{r}
longitudinal %>% select(MR.Delay, Age, EDUC, MMSE, eTIV, nWBV, ASF) %>% cor()
```

# ASF == Atlas Scaling Factor (unitless). Computed scaling factor that transforms native-space brain and skull to the atlas target (i.e., the determinant of the transform matrix).
# eTIV == Estimated total intracranial volume (cm3). Automated estimate of total intracranial volume in native space derived from the ASF.

# That is why both variables are so highly correlated
