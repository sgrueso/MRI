...
# The model has a 0.84 accuracy with only one parameter, that is unsual and very high accuracy. But can I improve the accuracy if the model includes more parameters like Age, Education or Social Economic Status?. The variables that seem 

# First we can try Random Forest. We get a better accuracy than just MMSE model.
...
```
# we see that EDUC and SES do not have importance to predict Dementia.

```{r}
set.seed(1, sample.kind="Rounding")

train_rf <- train(Group ~ Age + MMSE + eTIV + nWBV,
               method = "rf",
               data = train_set)
...
# We can Also try our model with the crossectional study database


```{r}
pred_rf <- predict(train_rf, cross)

acc_rf <- mean(pred_rf == cross$Group)
...
```{r}
fit <- randomForest(Group~MR.Delay + Age + EDUC + MMSE + SES + eTIV + nWBV, data = train_set)
confusionMatrix(predict(fit, test_set), test_set$Group)$overall["Accuracy"]
```
